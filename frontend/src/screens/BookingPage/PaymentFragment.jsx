import React, { useState } from 'react';
import Button from '../../components/Button/Button';
import PaymentCard from '../../components/PaymentCard/PaymentCard';
import Swal from 'sweetalert2';
import { Link } from 'react-router-dom';

const PaymentFragment = ({ incrementFragmentNo }) => {
  const [selectedMethod, setSelectedMethod] = useState(null);

  const showSuccessPopup = () => {
    Swal.fire({
      title: 'Payment Successful!',
      icon: 'success',
      confirmButtonColor: '#28a745', // Green color for the confirm button
      confirmButtonText: 'OK',
    });
  };

  const classicMethods = [
    {
      img: 'https://banner2.cleanpng.com/20180802/xri/kisspng-logo-mastercard-vector-graphics-font-visa-mastercard-logo-png-photo-png-arts-5b634298cd58d5.9008352515332317688411.jpg',
      name: 'MasterCard',
    },
    {
      img: 'https://w7.pngwing.com/pngs/618/512/png-transparent-visa-logo-mastercard-credit-card-payment-visa-blue-company-text.png',
      name: 'Visa',
    },
  ];
  const modernMethods = [
    {
      img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/46/Bitcoin.svg/1200px-Bitcoin.svg.png',
      name: 'Bitcoin Payment',
    },
    {
      img: 'data:image/png;base64,',
      name: 'Inter Galactic Coins',
    },
  ];
  return (
    <>
      <div className="my-8 mx-8">
        <h2 style={{ fontSize: '24px' }}>Payment Details</h2>
      </div>
      <div className="my-6 mx-8">
        <h6>Classic Methods</h6>
      </div>
      <PaymentCard paymentMethods={classicMethods} selectedMethod={selectedMethod} setSelectedMethod={setSelectedMethod} />
      <div className="my-6 mx-8">
        <h4>Modern Methods</h4>
      </div>
      <PaymentCard paymentMethods={modernMethods} selectedMethod={selectedMethod} setSelectedMethod={setSelectedMethod} />
      <div className="my-8 mx-8 flex justify-center gap-2 items-center">
        <div className="flex flex-col justify-center items-center">
          <h1 className="text-1xl">Total</h1>
          <h1 className="text-3xl">$900</h1>
        </div>
        <Link to="/">
          <Button text="Proceed to Payment" type="full" onClick={showSuccessPopup} />
        </Link>
      </div>
    </>
  );
};

export default PaymentFragment;
